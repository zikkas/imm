...
Let $(X,\|\cdot\|)$ be a Banach space and $\mathcal{L}(X)$ be the Banach space of bounded linear operators.
\begin{definition}
...
\begin{definition}
	...
	\begin{align*}
		&\rho(\mathscr{A})=\ \text{Resolvent of $\mathscr{A}$} = \left\{\lambda\in \mathbb{C}\ \bigg| \begin{aligned}
			...
			&(\lambda I - \mathscr{A})^{-1} \, \text{is bounded}
		\end{aligned}\right\}\\
...
\end{exercise}		
\begin{lemma}
	If an unbounded linear operator $\mathscr{A}$ is closable, then $\rho(\overline{\mathscr{A}})=\rho(\mathscr{A})$. If $\mathscr{A}$ is closed then $\rho(\mathscr{A})=\{\lambda\in \mathbb{C}\ | \ (\lambda I-\mathscr{A})^{-1}\in \mathcal{L}(X)\}$.\\
	\textcolor{blue}{Hints (Exercise): If $\rho(\mathscr{A})\neq \emptyset$ implies $\mathscr{A}$ is closed. (Show first that if $T$ is a UBLO with $T^{-1}$ bounded implies $T$ is closed)}.
\end{lemma}		
	...
	\begin{coro}
		...
		\begin{itemize}
			\item $\sigma_p(\mathscr{A})=\left\{\lambda\in\mathbb{C}:\ker(\lambda I-\mathscr{A})\neq \{0\}\right\}$ (punctual spectrum and $\lambda$'s are the eigenvalue).
			\item $\sigma_c(\mathscr{A})=\left\{\lambda\in\mathbb{C}:\ker(\lambda I-\mathscr{A})= \{0\},\overline{\text{Rg}(\lambda I-\mathscr{A})}\subsetneq X\right\}$ (continuous spectrum).
			\item $\sigma_r(\mathscr{A})=\left\{\lambda\in\mathbb{C}:\lambda I-\mathscr{A} \ \text{is injective},\overline{\text{Rg}(\lambda I-\mathscr{A})}= X, \ \text{Rg}(\lambda I-\mathscr{A})\neq X\right\}$ (residual spectrum).
		\end{itemize}
	\end{coro}
\begin{exercise}{}{}
...
\end{exercise}	
	\begin{theo}
		If $\mathscr{A}$ is a closed UBLO then $\rho(\mathscr{A})$ is open. If $\mu\in \rho(\mathscr{A})$, then for all $\lambda\in \mathbb{C}$ with $r:=\left|\mu-\lambda\right| \norm{(\mu I-\mathscr{A})^{-1}} < 1$ then $\lambda\in \rho(\mathscr{A})$ and
	\vspace{-2mm}
...
\begin{coro}
	...
	$
	from $\rho(\A)$ into $\L(X)$ is analytic. Moreover,
	\[
...
If $X$ is a Banach space with dual $X^*$, then for $x\in X$ and $x^*\in X^*$, we define the duality product as $\langle x^*,x\rangle$.
\begin{definition}[Dual Operator of $\mathscr{A}$]
Let
$
\A : D(\A) \subset X \to Y
$ (UBLO), be such that \\ $\overline{D(\A)}=X$.
...
\langle y^*, \A x \rangle = \langle z^*, x \rangle
\ \forall x \in D(\A)
\right\}.
\]
and $y^* \in D(\A^*)$, the element $z^*$ is unique and we define
$
\A^* y^* := z^*.
$	
\end{definition}		
\begin{lemma}
	Let $X, Y$ be Banach spaces and let $\mathscr{A} \in \mathcal{L}(X,Y)$.
	Then $\mathscr{A}^* \in \mathcal{L}(Y^*, X^*)$ and
	\vspace{-2mm}
	\[
	\|\mathscr{A}^*\|_{\mathcal{L}(Y^*,X^*)} = \|\mathscr{A}\|_{\mathcal{L}(X,Y)}.
	\]
\end{lemma}		
\begin{lemma}
...
\begin{definition}
Let $K:X\to Y$ be a BLO (in $\mathcal{L}(X,Y)$), then $K$ compact iff
$
K(B_1^X(0))$ \text{ is relatively compact in } Y (i.e. $\overline{K(B_1^X(0))}$ compact).
\vspace{-2mm}
\[
\mathcal{K}(X,Y)=\{K\in \mathcal{L}(X,Y)\mid K \text{ is compact}\}.
\]
\end{definition}
...
\end{exercise}
\begin{theo}
	$\mathcal{K}(X,Y)$ is a closed subspace of $\mathcal{L}(X,Y)$.
\end{theo}
...
Let	$X,Y$ and $Z$ be Banach spaces on $\mathbb{K}$.
	...
	\quad \mathscr{A}\in\mathcal{L}(X,Y),\ \mathscr{B}\in\mathcal{L}(Y,Z).
	\]
	\begin{itemize}
...
\begin{exercise}{}{}
	Let $X$ be a Banach space on $\mathbb{K}$. If $\mathscr{A}\in\mathcal{L}(X)$,
...
For $n=1,x\in \ker(\lambda I-\mathscr{A})\ \implies\ x=\frac{\mathscr{A}}{\lambda}x.$
For all $1\le m<n$,
\begin{align*}
...
With Proposition \ref{1.2.2} $\implies$ $\exists\, n_0$ (smallest one) such that
\vspace{-2mm}
\[
...
\]
\hspace{3mm}		
\begin{prop}[Fredholm Alternative]
...
\[
\dim X_n=n \ \implies\ \dim X_{n+1}=n+1.
\]
...
$\alpha_i=0$
$
\implies x_{n+1}=0, (\text{Impossible}).
$
...
\begin{definition}
Let $\A:D(\A)\subset X\to X$ UBLO, closed, $\rho(\A) \neq \emptyset$.
...
\begin{itemize}
	...
	\[
		\inf_{\substack{\norm{x} = 1 \\ x \in D(\A)}}\langle \A x,x\rangle
	\le \lambda \le
	\sup_{\substack{\norm{x} = 1 \\ x \in D(\A)}}\langle \A x,x\rangle.
	\]
\end{itemize}
...
\end{prop}
\begin{proof}
Since $\A$ is injective then $\A^{-1}$ is well defined, and since $\A = \A^{\circ}$ then $\A$ is closed.

Now assume $(x_n) \subset D(\A)$ such that $x_n \to x \in D(\A)$ (because $\A$ is closed) and $\A x_n -\to y$ then for all $z\in D(\A)$, $\langle\A x_n,z\rangle=\langle x_n,\A^{\circ}z\rangle$ which implies $\langle y,z\rangle=\langle x,\mathscr{A}^{\circ}z\rangle$ and so we have $x\in D(\mathscr{A}^{\circ})=D(\mathscr{A})$ and $y=\mathscr{A}^{\circ}$. Notice $\overline{\mathrm{Rg}(\mathscr{A})}=\ker(\mathscr{A})^{\perp}$ (because of self-adjointness).\\
...
= \langle u, w \rangle
\implies \forall x \in D(\mathscr{A})
\quad
\langle x, z \rangle
...
\implies x \perp Rg(\mathscr{A}) = \mathscr{H}.
\]
which implies $x=0$.\\
...
$\mathscr{A}$ surjective $\implies \exists w \in D(\mathscr{A})$
s.t.\ $\mathscr{A}w = y$ and $\mathscr{A}^{\circ}x = y$.\\
Since $\mathscr{A}$ is symmetric: $\mathscr{A}^{\circ}w = \mathscr{A}w$.
Then $\mathscr{A}^{\circ}w = \mathscr{A}^{\circ}x$, $\mathscr{A}$ is injective $\implies w = x$.
Hence $\mathscr{A}x = \mathscr{A}w = y \implies y = \mathscr{A}x \implies \mathscr{A}$ is closed.\\
By closed graph theorem both $\mathscr{A}$ and $\mathscr{A}^{-1} \in \mathcal{L}(X)$. We can conclude that $\mathscr{A}$ is a self-adjoint operator.
\end{proof}
\begin{exercise}{}{}
	Let $
...
\begin{itemize}
	\item [a.] $\inf\limits_{\substack{\norm{x}=1 \\ x \in D(\A)}} \langle \A x, x \rangle > -\infty$
	\item [b.] $\sup\limits_{\substack{\norm{x} = 1 \\ x \in D(\A)}} \langle \A x, x \rangle < +\infty$
	\end{itemize}
...
$(\impliedby)$ Let $x \in D(\mathscr{A})$, $x \neq 0$, and $\lambda > 0$.
...
\vspace{-3mm}
\[
\|(\lambda I - \mathscr{A})^{-1}\|_{\mathcal{L}(X)}
\le \frac{1}{\lambda}.
\tag{1.4}
\]
\end{theo}
...
Hence,
\[
\|(\lambda_0 I - \mathscr{A})^{-1}\|_{\mathcal{L}(X)}
\le \frac{1}{\lambda_0},
\qquad
...
\begin{definition}
	...
	\vspace{-3mm}
	 $$\|T(t)-Id\|_{{\mathcal{L}}(X)}\rightarrow 0 \quad \text{as} \ t\to 0^+$$
\end{definition}
...
$
\[
	s \geq t \;\implies\; T(s)=T(s-t)T(t)
	\implies\; T(s)-T(t)=T(t)\,[T(s-t)-I_d]
	\]
	\[
...
&\leq \sum_{k=0}^{n-1}\left\|T\left((n-k-1)\frac{t}{n}\right)\right\|\left\|T\left(\frac{t}{n}\right)-S\left(\frac{t}{n}\right) \right\|\left\|S\left(k\frac{t}{n}\right)\right\|\leq Cn\frac{\varepsilon}{TC}\frac{t}{n}\leq \varepsilon.
\end{align*}
...
\begin{enumerate}[label=\bfseries \thesection.\arabic*, 
	    ...
	    \item \label{sc:space_cont} $\forall x \in X, \forall t \ge 0, \; \lim\limits_{h \to 0} \frac{1}{h} \int_t^{t+h} T(s)x \, ds = T(t)x$ for all $x \in X$.
...
If $x \in D(\A)$, then 
$$
T(t)\A x = T(t) \lim_{h \to 0} \frac{T(h)x-x}{h} = \lim_{h \to 0} \frac{T(h)T(t)x - T(t)x}{h}
$$
This limit exists and equals $\A(T(t)x)$, proving $T(t)x \in D(\A)$ and $T(t)\A x = \A T(t)x$. This also shows the right-derivative of $T(t)x$ is $\A T(t)x$. A similar argument for the left-derivative completes the differentiability.
\end{proof}
...
Let $x_n \in D(\A)$ such that $x_n \to x$ and $\A x_n \to y$. By \ref{sc:img} we have $T(t)x_n - x_n = \int_0^t T(s)\A x_n \, ds$. Passing to the limit $n \to \infty$, we get $T(t)x - x = \int_0^t T(s)y \, ds$. Dividing by $t$ and letting $t \to 0^+$, the Right-Hand Side (RHS) converges to $y$. Thus $x \in D(\A)$ and $\A x = y$.
\end{proof}

\begin{theo}
	Let $\{T(t)\}_{t \ge 0}$ and $\{S(t)\}_{t \ge 0}$ be two $C_0$-SG with infinitesimal generators $\A$ and $\B$, respectively. If $\A = \B$, then $T(t) = S(t)$ for all $t \ge 0$.
\end{theo}

\begin{proof}
	Assume $\A = \B$, then $D(\A) = D(\B)$. Let $x \in D(\A)$ be fixed, and for a fixed $t > 0$, define
$$
...
\begin{proof}
Let $\mathscr{D} = \left\{\varphi:\mathbb{R} \to \mathbb{C} | \, \varphi \, \text{has compact support in } \mathbb{R}^{*}_{+} \, \text{and is smooth }(C_c^{\infty}) \right\}$. Let $x \in X$ and consider a test function $\varphi \in \mathscr{D}$. Define
$$
...
\end{align*}
As $h \to 0$, the quotient $\dfrac{\varphi(u-h) - \varphi(u)}{h}$ converges uniformly to $-\varphi'(u)$ because $\varphi$ is $C_c^{\infty}$. Thus:
$$
...
$$
This identity holds for all $C_c^{\infty}$ functions $\varphi$. Then $\langle x^*, T(s)x \rangle$ must be zero for all $s > 0$. 
...
\begin{theo}[Hille-Yosida Theorem (Contraction Case)]\label{thm:hy}
...
\begin{enumerate}[label=(\roman*)]
	...
	\item \label{hy:bound} $\mathbb{R}^*_+ \subset \rho(\A)$ and for all $\lambda > 0$: 
		$$
		\norm{R(\lambda, \A)} \le \frac{1}{\lambda}
		$$
\end{enumerate}
\end{theo}
...
To prove ($\impliedby$), we introduce a family of bounded operators that approximate the unbounded generator $\A$.
...
\begin{proof}[Proof of $(\impliedby)$ of Hille-Yosida Theorem \ref{thm:hy}]
...
This is possible because $\lim\limits_{\lambda \to +\infty} {\A_\lambda x} = \A x$.
...
This limit is well defined by the argument above for all $x \in D(\A)$. Moreover, since $D(\A)$ is dense in $X$ and $\norm{e^{tA_\lambda}} \le 1$, we can extend $T(t)$ to a bounded linear operator on all of $X$ by density.
...
\begin{enumerate}
    ...
    $$
\item Strong Continuity: For $x \in D(\A)$, convergence is uniform on compact intervals of $t$, because of lemma \ref{hy:uc}. Thus $t \mapsto T(t)x$ is continuous. By the density of $D(\A)$ and uniform boundedness $\norm{T(t)} \le 1$, continuity extends to all $x \in X$.
\end{enumerate}

We must show that the generator of the constructed semigroup $T(t)$ is indeed $\A$. Let $\B$ be the generator of $T(t)$, we show that $\A = \B$.
...
$$
Thus, $x \in D(\B)$ and $\B x = \A x$, implying $\A \subset \B$.
Since $\B$ is the generator of a $C_0$-semigroup of contractions, $1 \in \rho(\B)$. By hypothesis, $1 \in \rho(\A)$. Since $\A \subset \B$ and both $(I-\A)$ and $(I-\B)$ are surjective (mapping onto $X$), it follows that $\A = \B$.
\end{proof}
...
\begin{proof}
In the previous proof, we constructed a SG, let us call it $S(t)$, defined by $S(t)x = \lim\limits_{\lambda \to +\infty} e^{t \A_\lambda}x$. We proved that the generator of $S(t)$ is exactly the operator $\A$.
...
$$
Let $\B$ be the generator of $S(t)$. By the definition of the generator:
$$
\B x = \lim_{t \to 0^+} \frac{e^{-\omega t} T(t)x - x}{t} = \lim_{t \to 0^+} \left( e^{-\omega t} \frac{T(t)x - x}{t} + \frac{e^{-\omega t} - 1}{t} x \right) = \A x - \omega x
$$
Thus, $\B = \A - \omega I$.
Since $\B$ generates a contraction semigroup, by the Hille-Yosida Theorem for contractions (Case $M=1, \omega=0$), we know that for any $\mu > 0$, $\mu \in \rho(\B)$ and $\norm{R(\mu, \B)} \le \dfrac{1}{\mu}$.
...
\noindent $(\impliedby)$.
Conversely, suppose $\A$ satisfies conditions (i) and (ii). Define $\B = \A - \omega I$.
Clearly, $\B$ is closed and densely defined.
...
$$
Thus, $\B$ satisfies the Hille-Yosida conditions for the contraction case ($M=1, \omega=0$). Therefore, $\B$ generates a contraction semigroup $S(t)$ with $\norm{S(t)} \le 1$.
Defining $T(t) = e^{\omega t} S(t)$, we see that $T(t)$ is a $C_0$-semigroup generated by $\A = \B + \omega I$, and it satisfies:
$$
...
\begin{lemma} \label{re:norm}
	Let $A: D(\A) \subset X \to X$ be a linear operator with $\rho(\A) \supset \mathbb{R}_{+}^{*}$ such that for all $\lambda > 0$ and $n \ge 0$, $\norm{\lambda^n R(\lambda, \A)^n} \le M$.
	Then, there exists a norm $\norm{\cdot}_\mu$ on $X$ such that:
\begin{enumerate}
	\item The norms are equivalent: $\norm{x} \le \norm{x}_\mu \le M \norm{x}$ for all $x \in X$.
\item For all $\lambda > 0$, the operator $\lambda R(\lambda, \A)$ is a contraction in the new norm:
$$
...
\begin{enumerate}[label=(\roman*)]
	\item $\A$ is closed and ($\overline{D(\A)} = X$).
	\item $\rho(\A) \supset (\omega, \infty)$ and for every $\lambda > 0$:
    $$
    \norm{(R(\lambda, \A))^n} \le \frac{M}{(\lambda - \omega)^n}, \quad \forall n \in \mathbb{N}
    $$
\end{enumerate}
\end{theo}
...
\begin{align*}
	e^{t[(\A - \omega I)_{\lambda} + \omega I]}x - e^{t \A_{\lambda}}x
	&= e^{t[(\A - \omega I)_{\lambda} + \omega I - \A_{\lambda} + \A_{\lambda}]}x - e^{t \A_{\lambda}}x \\
...
Note that $e^{tH(\lambda)} = e^{t(\B_\lambda + \omega I)} e^{-t \A_\lambda}$. From the bound estimates shown earlier, both $\norm{e^{s(\B_\lambda + \omega I)}}$ and $\norm{e^{-s \A_\lambda}}$ are uniformly bounded for $s$ in a compact interval $[0, T]$ and sufficiently large $\lambda$. Let $C$ be this bound. Then:
$$
\norm{(e^{t H(\lambda)} - I) x} \le t \cdot C \cdot \norm{H(\lambda)x}
$$
Since we proved that $\lim_{\lambda \to \infty} \norm{H(\lambda)x} = 0$, it follows immediately that:
$$
\lim_{\lambda \to \infty} \norm{(e^{t H(\lambda)} - I) x} = 0
$$
Finally, combining this with the uniform bound on $\norm{e^{t \A_\lambda}}$, we obtain:
$$
\lim_{\lambda \to \infty} \norm{e^{t (\B_\lambda + \omega I)} x - e^{t \A_\lambda} x} = 0
$$
...
A major problem in semigroup theory is recovering $T(t)$ from $R(\lambda,\A)$ (Inverting the Laplace Transform).
...
\begin{proof}
	...
	$$
\mathscr{L}\{e^{tb}\}(\lambda) = \int_0^\infty e^{-\lambda t} e^{tb} \, dt = \frac{1}{\lambda - b}, \quad \text{for } \Re(\lambda) > \Re(b)
	$$
	...
	The case of the operator follows the same steps and reasoning.
